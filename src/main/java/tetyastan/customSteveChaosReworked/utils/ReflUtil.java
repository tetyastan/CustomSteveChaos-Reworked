...
public class ReflUtil {
	...
	public static void invokeMethod(Object object, String methodName, boolean isStatic, Class<?>[] classArgs, Object... args) {
		
		try {
			
			if(isStatic) {
				((Class<?>) object).getMethod(methodName, classArgs).invoke(null, args);
			} else {
				object.getClass().getMethod(methodName, classArgs).invoke(object, args);
			}
