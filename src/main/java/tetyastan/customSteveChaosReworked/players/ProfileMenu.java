...
public class ProfileMenu extends Menu {
	...
	public ProfileMenu(CustomPlayer p, Menu from) {
		super(Main.getInstance().getLanguage("menus.profile.name").replace("%player%", p.getBP().getName()), 54, true);
		
		this.from = from;
		
		inv.setItem(0, ItemsUtil.generateItem(Material.BIRCH_SIGN, Main.getInstance().getLanguage("menus.profile.back")));
		
		ItemStack empty = ItemsUtil.generateItem(Material.BLACK_STAINED_GLASS_PANE, (byte)14, " ");
